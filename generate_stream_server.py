# Generator script to create the complete stream_server.py
import os
...
STREAM_PORT = 15433
LOG_FILE = r"C:\\Users\\zhsha\\.copaw\\workspaces\\ymw7EE\\stream_debug.log"
...
def parse_qwen_tool_calls(content, tools=None):
    tool_calls = []
    pattern = r'{XML_START}\s*(.*?)\s*{XML_END}'
...
def sanitize_chunk(chunk, tool_call_state=None, tools=None):
    ...
    if tool_call_state is not None and content_str:
        ...
        if start_marker in buf and end_marker in buf:
            ...
            clean_delta["content"] = ""
    elif content_str and "{XML_START}" in content_str:
...
def stream_chat(messages, tools=None, tool_choice=None):
    if not DASHSCOPE_URL:
        ...
        yield b"data: [DONE]\\n\\n"
        return
    log(f"[REQUEST] model={{MODEL_NAME}}, msgs={{len(messages)}}, tools={{len(tools or [])}}, choice={{tool_choice}}")
    for msg in messages:
        if isinstance(msg, dict) and msg.get("role") == "assistant":
            if "reasoning_content" not in msg: msg["reasoning_content"] = ""
    tool_enforcement_msg = {{
        "role": "system",
        "content": "CRITICAL: Before calling any tool, verify ALL required arguments are provided and non-empty. Never return empty arguments for tools."
    }}
    messages = list(messages) + [tool_enforcement_msg]
    headers = {{
    ...
    try:
        ...
        if resp.status_code != 200:
            ...
            yield b"data: [DONE]\\n\\n"
            return
        tool_call_state = {{"_messages": messages, "_used_paths": set()}}
        for line in resp.iter_lines():
            ...
            try:
                ...
                sanitized = sanitize_chunk(chunk, tool_call_state, tools)
                yield f"data: {{json.dumps(sanitized)}}\\n\\n".encode("utf-8")
            except json.JSONDecodeError as e:
                log(f"[PARSE] SSE error: {{e}}")
                continue
...
with open(r'C:\Users\zhsha\.copaw\workspaces\ymw7EE\copilot-proxy-src\copilot_proxy\stream_server.py', 'w', encoding='utf-8') as f:
    f.write(code)

print("Generated stream_server.py successfully")
print(f"File size: {os.path.getsize(r'C:\Users\zhsha\.copaw\workspaces\ymw7EE\copilot-proxy-src\copilot_proxy\stream_server.py')} bytes")
